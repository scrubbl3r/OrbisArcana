...
if (!Array.isArray(wordbook.words)) {
  failCheck(CHECK_TAG, "master-control json wordbook.words must be an array");
}
...
if (!Array.isArray(spellbook.spells)) {
  failCheck(CHECK_TAG, "master-control json spellbook.spells must be an array");
}
if (stable(wordbook.words) !== stable(spellbook.spells)) {
  failCheck(CHECK_TAG, "master-control json wordbook.words must match compatibility spellbook.spells");
}
...
if (!Array.isArray(authoringRoot.words)) {
  failCheck(CHECK_TAG, "master-control authoring json must include canonical words[]");
}
if (Object.hasOwn(authoringRoot, "spells")) {
  if (!Array.isArray(authoringRoot.spells)) {
    failCheck(CHECK_TAG, "master-control authoring compatibility spells must be an array");
  }
  if (stable(authoringRoot.words) !== stable(authoringRoot.spells)) {
    failCheck(CHECK_TAG, "master-control authoring words[] must match compatibility spells[]");
  }
}

const markdown = readRelativeText(RULE_ENGINE_V2_DOC_PATHS.masterControlMarkdown);
if (!markdown.includes("## Wordbook (SSOT)")) {
  failCheck(CHECK_TAG, "master-control markdown must include canonical Wordbook section");
}
if (markdown.includes("## Spellbook (SSOT)")) {
  failCheck(CHECK_TAG, "master-control markdown must not use legacy Spellbook section heading");
}

reportCheckPass(
  CHECK_TAG,
  "master-control artifacts use canonical wordbook/words terminology with explicit compatibility aliases"
);
