...
const PASS_MESSAGE = "rule-engine preview system matches canonical word conditions as runtime signals";
...
function runScenario(ruleId, condition) {
  ...
  const system = createRuleEnginePreviewSystem({
    ...
    schema: {
      signals: [
        { id: SIGNAL_ID_RUNTIME_ROTA, sourceEvent: EVT_WORD_DETECTED, where: { path: CONDITION_WORD_ID_PATH, eq: WORD_ID_ROTA } },
      ],
      windows: [],
      events: [],
      rules: [
...
expectSingleMatch(
  "word.signal_prefix",
  runScenario(RULE_WORD_SIGNAL_PREFIX, { type: CONDITION_TYPE_WORD, id: SIGNAL_ID_RUNTIME_ROTA }),
  RULE_WORD_SIGNAL_PREFIX
);
expectSingleMatch(
  "word.bare",
