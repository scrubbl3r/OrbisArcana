  ...
  switch (String(baseEffect || "")) {
    ...
    case "orb-lifecycle-3d": {
      ...
      return [
        ...
        `  maxCracks: ${Math.round(toNum(p.orbLifecycle3dCrackTotal, 5))},`,
        `  crackColor: ${colorHex("orbLifecycle3dCrack", 0xf8fdff)},`,
        `  crackAlpha: ${clampNum(p.orbLifecycle3dCrackAlpha, 0, 1, 0.88).toFixed(2)},`,
        `  crackWidthPx: ${Math.max(0.25, toNum(p.orbLifecycle3dCrackStroke, 1.35)).toFixed(2)},`,
        `  crackLiftBO: ${clampNum(p.orbLifecycle3dCrackLift, 0, 0.2, 0.006).toFixed(3)},`,
        "  criticalGlow: 0.25,",
        `  energyColor: ${colorHex("orbLifecycle3dEnergy", 0x05070a)},`,
        "  mutationSpeed: 0.00,",
        "  mutationAmount: 0.00,",
        "  diffuseWash: 0.00,",
        "  edgeBrightness: 0.00,",
