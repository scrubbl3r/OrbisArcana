  ...
  switch (String(baseEffect || "")) {
    ...
    case "flame-aoe-3d": {
      const keys = [
        ...
        "wakeLeadingStrength", "wakeLeadingWidth",
    ...
    case "orb-lifecycle-3d": {
      ...
      return [
        ...
        `  maxCracks: ${Math.round(clampNum(p.orbLifecycle3dCrackTotal, 1, 12, 3))},`,
        "  crackColor: 0xffffff,",
        `  crackAlpha: ${clampNum(p.orbLifecycle3dCrackAlpha, 0, 1, 0.92).toFixed(2)},`,
        "  crackWidthPx: 1.50,",
        "  crackLiftBO: 0.000,",
        `  edgeLightBrightness: ${clampNum(p.orbLifecycle3dEdgeLightBrightness, 0, 3, 1).toFixed(2)},`,
        `  edgeLightRange: ${clampNum(p.orbLifecycle3dEdgeLightRange, 0.2, 6, 1.85).toFixed(2)},`,
        `  holeEdgeSoftness: ${clampNum(p.orbLifecycle3dHoleEdgeSoftness, 0.25, 8, 1).toFixed(2)},`,
        "  criticalGlow: 0.25,",
        "  energyColor: 0x05070a,",
        `  startHoleSizeMin: ${clampNum(p.orbLifecycle3dStartHoleSizeMin, 0.001, 0.08, 0.01).toFixed(3)},`,
        `  startHoleSizeMax: ${clampNum(p.orbLifecycle3dStartHoleSizeMax, 0.001, 0.12, 0.02).toFixed(3)},`,
        `  childHoleCountMin: ${Math.round(clampNum(p.orbLifecycle3dChildHoleCountMin, 0, 16, 4))},`,
        `  childHoleCountMax: ${Math.round(clampNum(p.orbLifecycle3dChildHoleCountMax, 0, 24, 9))},`,
        `  childHoleSizeMin: ${clampNum(p.orbLifecycle3dChildHoleSizeMin, 0.001, 0.08, 0.004).toFixed(3)},`,
        `  childHoleSizeMax: ${clampNum(p.orbLifecycle3dChildHoleSizeMax, 0.001, 0.12, 0.012).toFixed(3)},`,
        `  childHoleRangeMin: ${clampNum(p.orbLifecycle3dChildHoleRangeMin, 0.001, 0.5, 0.045).toFixed(3)},`,
        `  childHoleRangeMax: ${clampNum(p.orbLifecycle3dChildHoleRangeMax, 0.001, 0.8, 0.145).toFixed(3)},`,
        "  mutationSpeed: 0.00,",
        "  mutationAmount: 0.00,",
        "  diffuseWash: 0.00,",
        "  edgeBrightness: 0.00,",
