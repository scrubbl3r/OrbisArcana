...
export function createOpalescentOrbGlowMaterial(config = ORB_MATERIAL_CONFIG) {
  const opalescenceSpeed = Number(config.opalescenceSpeed) || 1;
  return new THREE.ShaderMaterial({
    transparent: true,
    depthWrite: false,
    ...
    fragmentShader: `
      ...
      void main() {
        vec3 viewDir = normalize(vViewPosition);
        float fresnel = pow(1.0 - max(dot(normalize(vNormal), viewDir), 0.0), ${Number(config.glowFresnelPower).toFixed(3)});
        float driftA = sin((vWorldPosition.x * ${Number(config.driftScaleX).toFixed(4)}) + uTime * ${(Number(config.driftRateA) * opalescenceSpeed).toFixed(4)}) * 0.5 + 0.5;
        float driftB = sin((vWorldPosition.y * ${Number(config.driftScaleY).toFixed(4)}) + uTime * ${(Number(config.driftRateB) * opalescenceSpeed).toFixed(4)} + ${Number(config.driftPhaseB).toFixed(4)}) * 0.5 + 0.5;
        float driftC = sin((vWorldPosition.z * ${Number(config.driftScaleZ).toFixed(4)}) + uTime * ${(Number(config.driftRateC) * opalescenceSpeed).toFixed(4)} + ${Number(config.driftPhaseC).toFixed(4)}) * 0.5 + 0.5;
        vec3 glow = mix(uCyan, uViolet, driftA);
        glow = mix(glow, uGold, driftB * ${Number(config.goldMix).toFixed(3)});
        glow = mix(vec3(0.78, 0.9, 1.0), glow, ${Number(config.glowPastelMix).toFixed(3)} + driftC * ${Number(config.glowDriftPastelMix).toFixed(3)});
        float alpha = ${Number(config.glowCenterAlpha).toFixed(4)} + fresnel * ${Number(config.glowRimAlpha).toFixed(3)};
