...
export function formatWorldWorkshopGeometryReadout(surface = null, metrics = null) {
  ...
  switch (String(surface.generator || "")) {
    ...
    case "orb-displacement-test":
      return `BO ${roundMetric(metrics.bo)}px / traveling equator ripple ${roundMetric(metrics.displacementWaveCount)} bands / depth ${roundMetric(metrics.displacementWaveDepth)}px / equator ${roundMetric((metrics.displacementEquatorAmplitude || 0) * 100)}% / poles ${roundMetric((metrics.displacementPoleAmplitude || 0) * 100)}% / speed ${roundMetric(metrics.displacementSpeedHz)}Hz`;
