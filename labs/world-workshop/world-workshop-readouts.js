...
export function formatWorldWorkshopGeometryReadout(surface = null, metrics = null) {
  ...
  switch (String(surface.generator || "")) {
    ...
    case "orb-displacement-test":
      return `BO ${roundMetric(metrics.bo)}px / traveling equator ripple ${roundMetric(metrics.displacementWaveCount)} bands / depth ${roundMetric(metrics.displacementWaveDepth)}px / equator falloff ${roundMetric(metrics.displacementEquatorFalloff)} / speed ${roundMetric(metrics.displacementSpeedHz)}Hz`;
