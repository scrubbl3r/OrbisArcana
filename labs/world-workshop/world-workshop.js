...
import { renderIonicPlinthPreview } from "./previews/ionic-plinth-preview.js?v=20260426c";
...
function renderSurfacePreview({
  ...
  const metrics = renderIonicPlinthPreview({
    root: previewRoot,
    orbDiameterPx: ORB_BASE_VISUAL_DEFAULTS.diameterPx,
  });
  if (geometryReadout && metrics) {
    geometryReadout.textContent = `BO ${metrics.bo}px / column ${metrics.columnWidth}px x ${metrics.columnDepth}px x ${metrics.columnHeight}px / cap ${metrics.capitalWidth}px / base ${metrics.baseWidth}px`;
  }
  if (materialReadout) {
    materialReadout.textContent = "Three.js mesh / opaque black faces / 1px white vector edges";
  }
