...
export function formatEnemyWorkshopSwarmReadout(surface = null) {
  ...
  return `gnats total ${roundMetric(swarm.gnatsTotal, 0)} / spawn ${roundMetric(swarm.spawnRadiusBo)} BO / size ${roundMetric(swarm.gnatSizeBo, 3)} BO / z ${roundMetric(swarm.zDepthBo, 1)} BO / base speed ${rangeText(swarm.baseSpeedBoPerSec)} BO/s`;
}
