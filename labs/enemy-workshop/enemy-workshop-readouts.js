...
export function formatEnemyWorkshopPersonalityReadout(surface = null) {
  ...
  return `speed ${rangeText(ranges.speed)}% / chance ${rangeText(ranges.wanderChancePerMinute)}/min / range ${rangeText(ranges.wanderRangeBo)} BO / stops ${rangeText(ranges.wanderStopSpacingBo)} BO / route ${rangeText(ranges.routeCommitment)}`;
}
