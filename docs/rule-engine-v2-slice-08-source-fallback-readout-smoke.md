...
- V2 adapter build failure: `V2 adapter (safe fallback)`
- V2 bootstrap disabled: `V2 bootstrap disabled (safe disabled)`
- V2 builder missing: `V2 adapter missing builder (safe disabled)`
...
2. Introduce a temporary V2 validation break (for example unsupported key), reload receiver, confirm:
   - app still boots
   - `Rules:` shows `V2 adapter (safe fallback)`
