...
## 5) Wake-Window Token Flow
- Open flat-spin wake-window flow and test wake-window tokens (`rota`, `sanctum`, `vectus`).
- Confirm expected wake-window token behavior and chip lighting.
- Confirm closing the wake-window flow resets wake-window token heard state.
