...
## Quick Start
1. Edit behavior rules in `src/content/interactions-v2/interaction-graph-v2.js`.
...
- Validation failures:
  ...
  - See **Generated Artifacts Quick Links** -> `Effective Rule Engine Snapshot`.
...
- `npm run smoke:milestone:v2`
  - Runs `ready:v2` and updates trend/history artifacts.
...
1. Authoring edits only in SSOT files (`interaction-graph-v2.js`, `wordbook-v2.js`) unless doing intentional plumbing.
...
- Add a new interaction chain:
  - edit `src/content/interactions-v2/interaction-graph-v2.js` -> add a `rules[]` entry with `on` + `open` + `trigger`.
