...
import { makeVoronoiLayout, createRng } from "./orb-lifecycle-vfx-runtime.js";
...
function mapCrackPoint(point, {
  radius,
  scale,
  lift,
  rotate = 0,
} = {}) {
  const rawX = (Number(point && point.x) || 0) * scale;
  const rawY = (Number(point && point.y) || 0) * scale;
  const x = (rawX * Math.cos(rotate)) - (rawY * Math.sin(rotate));
  const y = (rawX * Math.sin(rotate)) + (rawY * Math.cos(rotate));
  const z = Math.sqrt(Math.max(0, (radius * radius) - (x * x) - (y * y))) + lift;
...
export function createOrbLifecycle3dCracks({
  ...
  const layout = makeVoronoiLayout(seed, crackCount, { orbRadiusPx: 50 });
  const positions = [];
  const ratio = Math.max(0, Math.min(1, hits / total));
  const glow = 1 + (ratio * Math.max(0, resolved.criticalGlow - 1));
  const rotatedCopies = ratio >= 0.66 ? [0, 2.1] : [0];

  for (const rotate of rotatedCopies) {
    for (const edge of layout.edges || []) {
      const a = mapCrackPoint(edge.a, { radius, scale, lift, rotate });
      const b = mapCrackPoint(edge.b, { radius, scale, lift, rotate });
  ...
  const material = new THREE.LineBasicMaterial({
    ...
    linewidth: resolved.crackWidthPx,
    transparent: true,
  ...
  lines.name = "orb_lifecycle3d:crack_lines";
  group.add(lines);
  return group;
}
