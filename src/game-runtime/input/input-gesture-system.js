 ...
 * @property {(sample?: {shakeVal01?:number, atMs?:number}) => boolean} processShakeSample Returns `true` if a shake hit was registered.
...
export function createInputGestureSystem({
  ...
  function processShakeSample({ shakeVal01, atMs = nowMs() } = {}) {
    const now = Number(atMs) || nowMs();
