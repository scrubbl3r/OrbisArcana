...
import { renderOrbStage } from "../orb-stage/orb-stage.js?v=20260507q";
...
import { createOrbStageReceiverVfxDefaults, initOrbStageReceiverVfxRuntime } from "../orb-stage/orb-stage-vfx-runtime.js?v=20260507s";
import { createOrbStageActionBridge } from "../orb-stage/orb-stage-action-bridge.js?v=20260507c";
import { loadStagingInitModules } from "../load-staging-init-modules.js?v=20260507g";
...
globalThis.__orbisStagingShellRuntimeVersion = "20260507bc";
...
function bindShellStageActions(shellContext) {
  ...
  const onTryAgain = () => {
    ...
    stopShellLegacyDomOrbShatterShardSim(shellContext);
...
function stopShellLegacyDomOrbShatterShardSim(shellContext) {
  const runtime = shellContext && shellContext.runtime ? shellContext.runtime : null;
...
function spawnShellLegacyDomOrbShatterShardFx(shellContext, payload) {
  const runtime = shellContext && shellContext.runtime ? shellContext.runtime : null;
  const shellVfx = runtime && runtime.vfx ? runtime.vfx : null;
...
async function initShellReceiverHostRuntime(shellContext) {
  ...
  const assembly = await bootstrapShellReceiverHostRuntimeAssembly({
    ...
    shellHooks: {
      ...
      spawnLegacyDomOrbShatterShardFx: (payload) => spawnShellLegacyDomOrbShatterShardFx(shellContext, payload),
      ...
      stopLegacyDomOrbShatterShardSim: () => stopShellLegacyDomOrbShatterShardSim(shellContext),
