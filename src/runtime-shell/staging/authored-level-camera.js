import { resolveLevelCameraAnchor } from "../../game-runtime/level/resolve-level-spawn-point.js";
...
export function resolveStageCameraZoom(level = null, mode = "gameplay", fallback = 1) {
  ...
  const key = mode === "preview" ? "previewZoom" : "gameplayZoom";
  return Math.max(
    0.05,
...
export function resolveStageCameraFollowMode(level = null, mode = "gameplay", fallback = "follow_target_center") {
  const camera = level && typeof level.camera === "object" ? level.camera : null;
  const key = mode === "preview" ? "previewFollowMode" : "gameplayFollowMode";
