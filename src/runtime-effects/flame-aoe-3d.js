...
import { FLAME_AOE_3D_PRESET_DEFAULT } from "../vfx/presets/flame-aoe-3d-default.js?v=20260529122856";
  ...
  return new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        float sourcePhase = surfaceBirth * 0.26 + sin(p.x / max(1.0, uOrbRadius) * 2.4 + time * 0.31) * 0.035;
        vec2 lateral = vec2(-sourceFlow.y, sourceFlow.x) * sin(time * 0.47 + p.x / max(1.0, uOrbRadius) * 1.8 + surfaceDistance * 1.2) * 0.05;
        noisePos.xy -= sourceFlow * (time * 0.42 + surfaceDistance * 0.08 + sourcePhase);
        noisePos.xy += lateral;
        noisePos.z = time * 0.28 + sourcePhase * 0.65 + sin(time * 0.17) * 0.28;
