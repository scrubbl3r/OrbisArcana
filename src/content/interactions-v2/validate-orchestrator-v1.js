...
function getRuleEntries(target) {
  return Array.isArray(target?.[FIELD_RULES]) ? target[FIELD_RULES] : [];
}
...
function collectRuleOnEntries(errors, rule, onContext) {
  if (isStringOrArray(rule[FIELD_ON])) {
    return parseOnSelectorList(rule[FIELD_ON], { invalidAsEmptyObject: true });
  }
...
function pushSelectorReferenceErrors(errors, ruleContext, type, id, rawId) {
...
function validateOpenAction(errors, ruleId, openForValidation) {
...
function validateTriggerEntry(errors, triggerContext, rawTrigger) {
...
function validateRuleActions(errors, rule, ruleId, ruleContext) {
  ...
  if (openForValidation) {
    validateOpenAction(errors, ruleId, openForValidation);
  }
  ...
  if (hasNonEmptyArray(triggerEntries) || hasOpenAction) {
    ...
    for (const rawTrigger of triggerEntries) {
      validateTriggerEntry(errors, triggerContext, rawTrigger);
    }
    return;
  }
...
function validateRuleOnEntries(errors, ruleContext, onEntries) {
  ...
  for (const entry of onEntries) {
    ...
    if (pushDuplicateWhenSeen(
      ...
      seenOn,
      `${type}.${id}`,
      `${ruleContext} contains duplicate ${LABEL_ON_SELECTOR}: ${type}.${id}`
    )) {
      continue;
    }
    pushSelectorReferenceErrors(errors, ruleContext, type, id, rawId);
  }
}

function resolveRuleIdForValidation(errors, seenRuleIds, rule) {
...
function validateRuleEntry(errors, seenRuleIds, rawRule) {
  ...
  const ruleId = resolveRuleIdForValidation(errors, seenRuleIds, rule);
  if (!ruleId) return;
  ...
  const onEntries = collectRuleOnEntries(errors, rule, onContext);
  validateRuleOnEntries(errors, ruleContext, onEntries);
  validateRuleActions(errors, rule, ruleId, ruleContext);
}
...
function validateOrchestratorRoot(errors, target) {
  ...
  if (typeof target[FIELD_ENABLED] !== TYPE_BOOLEAN) {
    errors.push(`${ROOT_CONTEXT}.enabled must be boolean`);
  }
  if (getRuleEntries(target).length || Array.isArray(target[FIELD_RULES])) return true;
...
function validateDefaultsOpenSection(errors, defaultsOpen) {
...
function validateDefaultsRuleSection(errors, defaultsRule) {
  pushUnsupportedKeys(
    errors,
...
function validateDefaultsSections(errors, defaults) {
  ...
  validateOptionalObjectSection(defaults, DEFAULTS_KEY_OPEN, (defaultsOpen) =>
    validateDefaultsOpenSection(errors, defaultsOpen)
  );
  validateDefaultsTriggerSection(errors, defaults);
  validateOptionalObjectSection(defaults, DEFAULTS_KEY_RULE, (defaultsRule) =>
    validateDefaultsRuleSection(errors, defaultsRule)
  );
}

function validateRuleEntries(errors, target) {
  const ids = new Set();
  for (const rawRule of getRuleEntries(target)) {
...
export function validateOrchestratorV1(cfg) {
  ...
  validateRuleEntries(errors, target);
  return errors;
}
